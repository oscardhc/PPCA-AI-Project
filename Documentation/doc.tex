...
为了方便书写，假设输入层至隐层的连接权为$\MC v_{\MC ij}, i=1,2...d j=1,2...q$，隐层至输出层的连接权为$\MC w_{\MC ij}, i=1,2...q j=1,2...l$，且假设隐层的输入为$\alpha_i i=1,2,...q$，输出层的输入为$\beta_i i=1,2,...l$。

对于一个训练例$(\MC x_{k},\MC y_{k})$，神经网络的输出为$\hat{\MC y_{k}}=(\hat{\MC y_1^k},\hat{\MC y_2^k}...\hat{\MC y_l^k})}$。则网络的均方误差为

$$\mathrm E_{k}=\frac{1}{2}\sum_{j=1}^{l}{\hat{(\MC y_j^k}-\MC y_j^k)^2}$$

任意参数$\MC v$的更新估计式为：
$\MC v \leftarrow \MC v + \Delta\MC v$

下面以隐层到输出层的连接权$\MC w_{\MC hj}$为例描述这一算法：

BP算法基于梯度下降策略，以目标的负梯度方向对参数进行调整。对于一个给定的学习率$\eta$（用于衡量每次梯度下降的程度，在0至1之间），有

$$\Delta\MC w_{\MC hj}=-\eta\frac{\partial\mathrm E_{k}}{\partial\MC w_{\MC hj}}$$

注意到$\MC w_{\MC hj}$先影响道第$\MC j$个输出层神经元的输入值$\beta_{j}$，再影响到其输出值$\hat{\MC y_{j}^{k}}$，然后影响到$\mathrm E_{k}$，有

$\frac{\partial\mathrm E_{k}}{\partial\MC w_{\MC hj}}=\frac{\partial\mathrm E_k}{\partial\hat{\MC y_{j}^{k}}}\cdot\frac{\partial\hat{\MC y_{j}^{k}}}{\partial\beta_{j}}\cdot\frac{\partial\beta_{j}}{\partial\MC w_{\MC hj}}$

由定义有$\frac{\partial\beta_{j}}{\partial\MC w_{\MC hj}}=\MC b_{h}$

Sigmoid函数在任意位置的导数均存在且可以被解出（其余连续的激活函数均满足这一要求）。有

$$\MC g_{j}=-\frac{\partial\mathrm E_{k}}{\partial\hat{\MC y_{j}^{k}}}\cdot\frac{\partial\hat{\MC y_{j}^{k}}}{\partial\beta_{j}}=-(\hat{\MC y_{j}^{k}}-\MC y_{j}^{k})\cdot\MC f'(\beta_{j}-\theta_{j})$$

则$\MC w_{\MC hj}$的更新公式为$$\Delta\MC w_{\MC hj} = \eta\MC g_j\MC b_h$$
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.8\textwidth]{assets/11}
	\includegraphics[width=0.8\textwidth]{assets/11-2}
	\caption{}
\end{figure}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.8\textwidth]{assets/12}
	\includegraphics[width=0.8\textwidth]{assets/12-2}
	\caption{}
\end{figure}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.6\textwidth]{assets/13}
	\caption{}
\end{figure}
